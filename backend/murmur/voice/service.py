...
class VoiceService:
    ...
    def __init__(
        ...
        *,
        ...
        self.tts_pipeline: TTSPipeline | KokoroTTSPipeline | None = None
        self.tts_fallback_pipeline: KokoroTTSPipeline | None = None
    ...
    def start(self) -> None:
        ...
        try:
            ...
            if config.TTS_PROVIDER == "kokoro":
                self.tts_pipeline = KokoroTTSPipeline(model_path=config.KOKORO_MODEL_PATH)
            ...
            self.tts_fallback_pipeline = None
...
def _get_kokoro_fallback_pipeline(service: VoiceService) -> KokoroTTSPipeline | None:
    """Lazily initialize a Kokoro fallback pipeline when enabled."""
    if not config.TTS_FALLBACK_TO_KOKORO:
        return None
    if isinstance(service.tts_pipeline, KokoroTTSPipeline):
        return service.tts_pipeline
    if service.tts_fallback_pipeline is None:
        try:
            service.tts_fallback_pipeline = KokoroTTSPipeline(model_path=config.KOKORO_MODEL_PATH)
            logger.info("Kokoro fallback TTS pipeline initialized")
        except Exception as e:
            logger.warning("Failed to initialize Kokoro fallback pipeline: %s", e)
            service.tts_fallback_pipeline = None
    return service.tts_fallback_pipeline
...
def _touch_voice_session(service: VoiceService, pc_id: str) -> None:
    service.runtime.touch_voice(pc_id)
...
async def _process_user_turn(service: VoiceService, pc_id: str, user_text: str):
    """
...
async def _run_sdl_step_pipeline(service: VoiceService, pc_id: str, sdl: dict):
    """
...
async def _run_llm_tts(service: VoiceService, pc_id: str, user_text: str):
    """
    ...
    async def _tts_sender():
        ...
        async def _stream_sentence_with_resilience(sentence: str) -> dict[str, Any]:
            ...
            primary_pipeline = service.tts_pipeline
            ...
            fallback_pipeline = _get_kokoro_fallback_pipeline(service)
        ...
        while True:
            ...
            if not service.tts_pipeline and not _get_kokoro_fallback_pipeline(service):
                continue
    ...
    try:
        ...
        if pending_sdl and pending_sdl.get("steps"):
            ...
            await _run_sdl_step_pipeline(service, pc_id, pending_sdl)
            ...
            if st_session:
                ...
                if speech_final and st_session.accumulated_transcript.strip():
                    ...
                    if is_complete and accumulated_text:
                        await _process_user_turn(service, pc_id, accumulated_text)
                ...
                if (is_final or speech_final) and transcript.strip():
                    ...
                    timing["stt_final_ts"] = time.perf_counter()
                    await _process_user_turn(service, pc_id, transcript)
...
async def _negotiate_offer(
    ...
    def on_track(track):
        ...
        if track.kind == "audio":
            task = asyncio.create_task(consume_audio_track(service, track, pc_id))
