...
interface OutboundRtpEvidence {
  readonly sender_count: number;
  readonly report_count: number;
  readonly bytes_sent: number;
  readonly packets_sent: number;
}

interface CompletedProof {
  readonly snapshot: BrowserSnapshot;
  readonly outboundRtp: OutboundRtpEvidence;
}
...
async function readOutboundRtp(
  page: Page,
  trackId: string
): Promise<OutboundRtpEvidence> {
  return page.evaluate(async (publishedTrackId) => {
    const diagnostic = (
      window as Window & {
        __murmurVoiceE2EReadOutboundRtp?: (
          id: string
        ) => Promise<OutboundRtpEvidence>;
      }
    ).__murmurVoiceE2EReadOutboundRtp;
    if (!diagnostic) throw new Error("Outbound RTP diagnostic was not installed");
    return diagnostic(publishedTrackId);
  }, trackId);
}

async function waitForProof(page: Page, timeoutMs: number): Promise<CompletedProof> {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    await assertNoHarnessError(page);
    const snapshot = await readSnapshot(page);
    const trackId = snapshot.local_track?.id;
    if (proofReady(snapshot) && trackId) {
      const outboundRtp = await readOutboundRtp(page, trackId);
      if (outboundRtp.bytes_sent > 0 && outboundRtp.packets_sent > 0) {
        return { snapshot, outboundRtp };
      }
    }
...
test("real browser media crosses LiveKit and barge-in stops the first reply", async ({
  ...
  await page.addInitScript(() => {
    ...
    Object.defineProperty(window, "__murmurVoiceE2EReadOutboundRtp", {
      configurable: false,
      value: async (publishedTrackId: string): Promise<OutboundRtpEvidence> => {
        let senderCount = 0;
        let reportCount = 0;
        let bytesSent = 0;
        let packetsSent = 0;
        for (const connection of connections) {
          for (const sender of connection.getSenders()) {
            if (sender.track?.id !== publishedTrackId) continue;
            senderCount += 1;
            const report = await sender.getStats();
            report.forEach((entry: unknown) => {
              const stat = entry as Partial<RTCOutboundRtpStreamStats>;
              if (stat.type !== "outbound-rtp") return;
              reportCount += 1;
              bytesSent = Math.max(bytesSent, stat.bytesSent ?? 0);
              packetsSent = Math.max(packetsSent, stat.packetsSent ?? 0);
            });
          }
        }
        return {
  ...
  const outboundRtp = completedProof.outboundRtp;
  ...
  expect(outboundRtp.sender_count).toBeGreaterThan(0);
  expect(outboundRtp.report_count).toBeGreaterThan(0);
  expect(outboundRtp.bytes_sent).toBeGreaterThan(0);
  expect(outboundRtp.packets_sent).toBeGreaterThan(0);
  ...
  writeResultAtomically(resultPath, {
    ...
    browser_evidence: {
      ...
      outbound_bytes_sent: outboundRtp.bytes_sent,
      outbound_packets_sent: outboundRtp.packets_sent,
