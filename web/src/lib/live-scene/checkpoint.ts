...
export const CHECKPOINT_CERTIFICATE_VERSION = 2 as const;
export const CHECKPOINT_COMPILER_VERSION =
  "murmur.completing_square_choreography.v1" as const;
export const CHECKPOINT_CERTIFICATE_ISSUER = "semantic_compiler" as const;
export const CHECKPOINT_RECEIPT_ISSUER =
  "completing_square_verifier" as const;
export const CHECKPOINT_CANONICALIZATION = "murmur-json-v1" as const;
export const CHECKPOINT_HASH_ALGORITHM = "sha256" as const;

export const COMPLETING_SQUARE_CHECKPOINT_IDS = [
  "problem",
  "area_model",
  "split_linear_term",
  "rearrange_halves",
...
export type CompletingSquareCheckpointId =
  (typeof COMPLETING_SQUARE_CHECKPOINT_IDS)[number];

export const CHECKPOINT_VERIFICATION_OBLIGATIONS = [
  "stable_id",
  "unique_ids",
  "board_bounds",
  "component_ownership",
...
export type CheckpointVerificationObligation =
  (typeof CHECKPOINT_VERIFICATION_OBLIGATIONS)[number];

export interface CheckpointVerificationReceiptV2 {
  readonly issuer: typeof CHECKPOINT_RECEIPT_ISSUER;
  readonly componentId: string;
  readonly checkpointId: CompletingSquareCheckpointId;
  readonly operationTargets: readonly string[];
  readonly obligationCodes: readonly CheckpointVerificationObligation[];
  readonly verified: true;
}

export interface CheckpointCompilerCertificateBodyV2 {
  readonly v: typeof CHECKPOINT_CERTIFICATE_VERSION;
  readonly issuer: typeof CHECKPOINT_CERTIFICATE_ISSUER;
  readonly compilerVersion: typeof CHECKPOINT_COMPILER_VERSION;
  readonly canonicalization: typeof CHECKPOINT_CANONICALIZATION;
  readonly hashAlgorithm: typeof CHECKPOINT_HASH_ALGORITHM;
...
export interface CheckpointCompilerCertificateV2 {
  readonly body: CheckpointCompilerCertificateBodyV2;
  readonly certificateSha256: string;
}

export interface CompiledCheckpointV2 {
...
function fail(
  ...
  throw new LiveSceneProtocolError(code, `checkpoint ${message}`);
}
...
function decodeReceipt(value: unknown): CheckpointVerificationReceiptV2 {
  ...
  if (input.obligationCodes.length === 0) {
    fail("verification receipt obligationCodes must be non-empty");
  }
...
function decodeCertificateBody(
  ...
  if (resultRevision !== baseRevision + 1) {
    fail(
      "certificate body revisions must advance exactly once",
      "revision_mismatch",
    );
  }
  ...
  return Object.freeze({
    ...
    presentationCheckpoint: decodePresentationCheckpointV1(
...
function operationTarget(operation: ScenePatchDraft["operations"][number]): string {
...
/**
 * Decode one server-certified checkpoint envelope.
 *
 * This validates its closed wire shape and cleartext relational bindings. The
 * SHA-256 fields are opaque backend-issued commitments here: browser decoding
 * deliberately does not claim cryptographic verification or recompute them.
 */
export function decodeCompiledCheckpointV2(value: unknown): CompiledCheckpointV2 {
  ...
  const beat = decodeRoutedChoreographyBeatV2(input.beat);
...
/** Parse JSON and then apply the same strict checkpoint validation. */
export function parseCompiledCheckpointV2(data: string): CompiledCheckpointV2 {
  let value: unknown;
  try {
  ...
  return decodeCompiledCheckpointV2(value);
}
