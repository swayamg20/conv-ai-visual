...
Murmur flips this. Every conversation has a **live canvas** — a visual surface where the AI places diagrams, charts, equations, and animations as it talks. When you ask "explain how DNS works", you don't just hear an explanation — you watch it build a diagram, animate packet flows, and label each step.
...
3. **Deepgram endpointing + Smart Turn** figures out when you're actually done talking (not just pausing)
4. **LLM** (OpenAI, Groq, or Gemini) reasons about what to do — respond, call a tool, draw something
5. **Tool execution** invokes trusted, bounded handlers — the AI can fetch data, search, compute, and draw on the canvas
6. **ElevenLabs TTS** streams the response back as speech, sentence by sentence (not waiting for the full response)
7. **Canvas updates** flow over SSE for chat and the WebRTC data channel for voice — Rough.js renders hand-drawn-style shapes and GSAP animates them

You can interrupt mid-sentence. Browser-side Silero VAD detects speech during playback, asks the server to cancel the active turn, and the pipeline pivots to the new input.
...
- **Retrieval quality** — move beyond keyword-only agent resources when evaluations prove the gain
- **Adaptive tutoring** — use mastery and session evidence to choose explanation depth and practice
