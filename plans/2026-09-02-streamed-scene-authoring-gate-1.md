...
- [ ] Implement strict backend scene-node, patch, request, and event contracts plus an incremental NDJSON parser.
- [ ] Implement an injectable scene-generation service and authenticated SSE route with authoritative revisions, one bounded repair stream, cancellation cleanup, and latency metrics.
- [ ] Implement strict frontend patch decoding/application, robust SSE framing, and a single-flight generation runtime over the Gate 0 motion executor.
- [ ] Add the authenticated product demo and an explicitly enabled, development-only fixture lab for auth-independent browser evaluation.
- [ ] Validate deterministic normal, invalid/repair, terminal failure, interruption, stale-output, replay, desktop, and narrow-viewport behavior.
- [ ] Run a small live-provider corpus only after the user supplies the repository-required explicit cost budget.
- [ ] Record measured outcomes, update this plan, commit focused checkpoints, push the branch, and verify the remote SHA.
...
Gate 1 is in progress. This section will record the exact commits, validation counts, latency measurements, model-validity corpus, user-visible behavior, remaining risks, and pass/fail verdict. Until a real-provider corpus runs under an explicit budget, the implementation may be complete but the model-quality portion of the gate remains unverified.
