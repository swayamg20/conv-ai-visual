...
- [x] 2026-09-20 17:30 IST: Split Firebase deployment authority from runtime token verification and required a separate deploy-only administrator whenever the frontend domain needs mutation.
...
- [x] 2026-09-20 19:35 IST: Finished independent release-blocker review and validation for exact Firebase Viewer permission attestation, serialized version-pinned secret cutover, post-health finalization, and recoverable retirement of every legacy Firebase admin-secret version. The focused deployment suite passes 61/61, Ruff and diff checks pass, both Bicep templates compile, and the live-resource-group `what-if` succeeds without deletions.
- [x] 2026-09-21 01:01 IST: Re-verified the public release `46b83de1e469f0c8d396be2dbeda192a710206da`. Frontend revision `murmur-web--0000004` and backend revision `murmur-api--0000004` are healthy, active, serving 100% traffic, and report the expected release SHA. `/healthz` and `/readyz` pass; database, Firebase, and Azure OpenAI readiness are all `ready` with zero paid model calls.
- [x] 2026-09-21 01:09 IST: Re-ran isolated browser QA against Azure. The landing page, `#demo` content, login page, and unauthenticated storyboard redirect render without console errors on desktop, tablet, and mobile. All observed page assets returned HTTP 200.
- [x] 2026-09-21 01:09 IST: Merged PR #41 after all ten GitHub checks passed. The merge adds the deployment-verifier regression for Azure's healthy `RunningAtMaxScale` state; it does not change the currently deployed application bytes.
- [x] 2026-09-21 12:05 IST: Measured the reported slowness against live Azure logs and browser timings. The web cold path is 16-21 seconds and the API cold path is 25-30+ seconds because both apps scale to zero; warm browser load is 0.42 seconds and warm API health is 0.11 seconds. The landing hero also hides critical content until hydration and deliberately reveals it over roughly 1.3-2.6 seconds.
- [x] 2026-09-21 15:29 IST: Set both Container Apps to an enforced `1..1` scale range, made critical hero content visible in server-rendered HTML, shortened only the decorative opening sketch to roughly one second, and stopped preloading the two non-critical handwriting fonts.
- [x] 2026-09-21 15:30 IST: Passed Ruff, deployment-driver tests, Bicep compilation, frontend lint, typecheck, all 1,300 frontend tests, production build, no-JavaScript hero visibility, reduced-motion behavior, and normal-motion browser checks.
- [x] 2026-09-21 15:34 IST: Prepared separate substantive frontend and infrastructure commits so each performance change remains independently reviewable.
- [ ] Push the performance branch and merge its reviewed pull request after required checks pass.
- [ ] Merge the performance pull request, deploy the exact merge SHA through the guarded driver, and prove both apps remain warm after the Azure scale-down window with zero paid model calls.
...
- [ ] If exact current-`main` provenance is required, rebuild the unchanged application images from merge commit `35ed53953dcce7e3ba5e6c90cf3eed29d23f3520` and verify that SHA. The live application remains release `46b83de1e469f0c8d396be2dbeda192a710206da`; the only newer merged change is verifier code.
...
- A healthy single-replica Container App at its configured maximum may report `RunningAtMaxScale` rather than `Running`. The verifier now accepts either state only while the revision is active and healthy, with regression coverage.
- Scale-to-zero kept idle cost bounded but produced a roughly 21-second first frontend response during the 2026-09-21 public re-check. Once warm, the backend health endpoint answered in 0.114 seconds. This is an explicit pilot latency tradeoff, not a failed deployment.
- The backend cold path is materially slower than the public page: its 241 MB image takes roughly 7-9 seconds to pull and readiness arrives roughly 25-30 seconds after activation. If the frontend and API wake serially, a first fully usable interaction can take 40-50 seconds even though both warm handlers are fast.
- The repository-wide Python suite cannot collect three unrelated voice modules in this isolated deployment worktree because the optional `livekit` package is absent. The 73 deployment and chat tests relevant to this change pass; CI remains the broad-suite authority with the complete dependency profile.
...
- The new runtime credential cannot read the project's IAM policy, but Google Cloud Resource Manager `testIamPermissions` returns its effective Firebase Authentication permissions. The live probe returned exactly `firebaseauth.configs.get` and `firebaseauth.users.get`, allowing the driver to reject both missing Viewer access and any known Authentication admin permission.
- The existing vault contains six enabled versions of each protected secret and a direct `Key Vault Secrets Officer` assignment created for the signed-in deployer during the original rollout. The corrected driver refuses that persistent writer grant; the exact stale assignment must be removed once before the new self-revoking flow can run.
- A versioned Key Vault reference prevents one revision from silently consuming another release's credential, but it does not serialize the multi-command Azure rollout. A fixed ARM deployment name is not a transaction lock, so the deployment needs a renewable finite Blob lease before credentials or application state change.
...
- 2026-09-20, Codex: Store the Azure model key and Firebase service-account JSON in Azure Key Vault. Each Container Apps backend revision reads exact version-pinned Key Vault references through its managed identity, so secrets do not enter source control, image layers, or normal deployment output and cannot silently change beneath a running revision.
...
- 2026-09-21, Swayam Gupta and Codex: Supersede the scale-to-zero pilot decision after live acceptance exposed 16-30+ second cold starts. Keep both frontend and backend at `minReplicas: 1` and `maxReplicas: 1`. This accepts continuous idle allocation for predictable MVP latency without changing ingress, identity, secret, model-call, or concurrency boundaries. Local SQLite remains non-durable across restart, reschedule, or deployment.
...
- 2026-09-20, Codex: Store the dedicated Viewer credential under a new `firebase-runtime-service-account-json` name so it never shares version history with the prior administrator credential. A separate optional domain administrator may update the authorized hostname but is never serialized into Key Vault. Stage new values once, cut over and verify the healthy revision, then disable old versions and every version of the legacy Firebase secret with exact readback. Disabled legacy values remain recoverable to privileged Azure operators; Google key revocation is a separate operator action.
- 2026-09-20, Codex: Serialize deployments with a 60-second renewable lease on one private Blob container scoped only to the deployment operator. Bind every backend revision to the exact two staged Key Vault versions, and finalize only while the lease is healthy and all older revisions have zero replicas in a terminal inactive state. Finite expiry recovers a crashed deploy without an automatic lease break.

## Outcomes & Retrospective

The isolated Azure foundation, ACR images, Key Vault references, Firebase authorized domain, and both Container Apps are live in Central India. The backend and frontend answer at their public HTTPS origins from release `46b83de1e469f0c8d396be2dbeda192a710206da` on revisions `murmur-api--0000004` and `murmur-web--0000004`; health, readiness, CORS, release provenance, probes, scale bounds, and current-ready revisions passed with zero paid model calls. Browser QA proved the landing page, complete demo section, sign-in page, responsive layouts, and protected storyboard redirect with no console errors.

The pilot is intentionally non-durable: backend SQLite lives on local ephemeral Container Apps storage because live testing proved that SQLite locking on the Azure Files SMB mount is not safe for this workload. PostgreSQL remains required before stored agents or history can be relied on across scale-down or deployment. PR #41 is merged and all ten checks passed. The remaining acceptance steps are a user-authenticated storyboard page load and, only if exact current-`main` provenance is required, a final rebuild from merge commit `35ed53953dcce7e3ba5e6c90cf3eed29d23f3520`; that merge changed verification code, not application behavior.
