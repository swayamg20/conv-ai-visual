...
- [x] 2026-09-08 03:53 IST: Defined the completing-square semantic spine as eight ordered main checkpoints, exact two-checkpoint stage prefixes, one closed `corner_detail` identity, nullable interruption frontiers, and an immutable `cornerClarified` state bit. All nine unclarified frontiers and every legal clarified frontier are covered; 151 combined choreography, semantic-state, and legacy semantic-contract tests pass.
- [x] 2026-09-08 03:54 IST: Mirrored the server choreography vocabulary in an independent TypeScript decoder with exact-key rejection, deep freezing, matching budgets, canonical cue/target order, viewport bounds, and the same five easing tokens. The complete frontend unit suite passes (45 files, 547 tests), with ESLint and TypeScript clean.
- [x] 2026-09-08 04:00 IST: Extended the shared semantic scene with a kind-discriminated completing-square state while locking the legacy Pythagorean dump and digest. Mixed component kinds round-trip, cross-kind duplicate IDs fail, and the V1 compiled-beat path rejects a target owned by the new kind while preserving unrelated new-kind components. The semantic/choreography/state suite passes (356 tests).
- [x] 2026-09-08 04:10 IST: Added the separate V2 checkpoint integrity envelope: ordered multi-operation verifier receipts, canonical low-level scene hashes, a domain-separated compiler certificate that embeds the full presentation checkpoint, and a compiled-checkpoint contract binding the routed beat, caption, operation targets, receipt, choreography, and certificate. Fixed vectors and mutation tests pass alongside all legacy semantic integrity tests (180 tests).
...
- The existing 64 KiB limit is the model NDJSON frame budget; the semantic SSE encoder independently permits 96 KiB events. Gate 1.5 will enforce its stricter 64 KiB compiled-checkpoint budget at the V2 compiler boundary without changing either shared V1 constant.
...
- 2026-09-08, Codex: Use only non-overshooting motion curves in the choreography vocabulary: linear, ease-in, ease-in-out, ease-out-quart, and ease-out-quint. Bounce, elastic, and back easings draw attention to the tween rather than the mathematical relationship and are excluded. Reduced motion preserves every checkpoint while removing spatial travel.
- 2026-09-08, Codex: Represent each flagship equation term as a measured `latex_token` whose `x` is a start, middle, or end horizontal anchor and whose `y` is its top edge. Preserve the legacy monolithic `latex` node exactly; token identity and explicit boxes enable deterministic FLIP motion without turning Gate 1.5 into general LaTeX morphing.
...
The first five implementation milestones are pushed as `96d7c07` (`feat: define live choreography contracts`), `305274a` (`feat: add measured equation token nodes`), `968a3f5` (`feat: define completing square checkpoints`), `e503858` (`feat: decode certified choreography plans`), and `9765063` (`feat: add completing square semantic state`). Together they establish the routed-only compiler boundary, deterministic presentation vocabulary, stable measured-token renderer, exact semantic lesson frontier, matching browser decoder, and a V1-compatible shared semantic scene. The full backend suite passes after those changes (1,806 tests), as does the full frontend suite (45 files, 547 tests), with Ruff, formatting, ESLint, TypeScript, and repository diff checks clean. The gate is not yet testable end to end: compiler, verifier, browser checkpoint playback, and evidence artifacts remain in progress. At completion this section must state the merged commit, CI run, real-speed artifact hashes, rubric result, and remaining product boundary.
