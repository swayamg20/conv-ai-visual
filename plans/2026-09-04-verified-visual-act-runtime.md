...
This plan builds a narrow Gate 1.1 prototype in which the model describes what to teach rather than how to draw it. A user will be able to reveal a Pythagorean area proof through compact semantic teaching beats. A deterministic compiler will own coordinates, stable child identities, styles, and mathematical relationships. An independent verifier will reject a realization whose geometry, equation, bounds, or identifiers do not satisfy the component's obligations. The compiler will lower each beat into separately committable visual atoms that reuse the existing `ScenePatchDraft` contract and existing SVG/GSAP runtime.

The key research hypothesis is not that streaming diagrams, semantic scene graphs, or interruptible AI tutors are new. A2UI and OpenUI already stream catalog-constrained UI; Penrose already separates mathematical content from visual style; Gemini already represents animation as declarative steps; DrawDash already applies speech-driven diagram suggestions; and PapinAI publicly claims a named-object whiteboard that draws with narration and survives learner interruption. The narrower hypothesis is that a **verified visual-act runtime** can give generative teaching an exact semantic commit frontier. A **teaching beat** is a compact model-authored request such as “derive the three area squares.” A **visual atom** is one compiler-authored, independently meaningful scene mutation such as revealing the square on side `a`. A **commit frontier** is the greatest ordered prefix of visual atoms the renderer has acknowledged as materialized. On interruption, later atoms are abandoned and the next generation is based only on that committed semantic prefix.
...
- [ ] Define strict model-authored teaching-beat contracts and server-owned semantic state/receipt contracts in `backend/murmur/live_scene/semantic_contracts.py`.
- [ ] Implement deterministic Pythagorean lowering and an independent realization verifier in `backend/murmur/live_scene/semantic_compiler.py`.
- [ ] Prove compactness, determinism, mathematical invariants, stable IDs, atomic failure, and interruption-prefix reconstruction with focused backend tests.
...
- 2026-09-04, Codex: Start with one executable concept, `pythagorean_area_proof`, instead of designing an open-domain ontology. The proof is rich enough to test geometry, labels, derivation, staged reveal, stable identity, and interruption while remaining independently verifiable.
...
No implementation outcome exists yet. The immediate deliverable is a provider-free Pythagorean vertical slice plus evidence that every visible atom has a stable semantic identity and verified realization. This section must be updated after each pushed milestone with exact commits, test results, and any change to the hypothesis.
...
The first semantic component is `pythagorean_area_proof`. Its compiler-owned roles are ordered as triangle, square on side `a`, its `a²` label, square on side `b`, its `b²` label, square on side `c`, its `c²` label, and the derived identity. A target stage reveals a prefix of those roles. This deliberately makes every renderer acknowledgement correspond to one independently meaningful semantic advance.

## Plan of Work

First, add `semantic_contracts.py` with two separate trust surfaces. `TeachingBeatDraft` is the small object a model may author. For this prototype it contains schema version, beat ID, short narration, a constrained teaching-act enum, and one typed `PythagoreanAreaProofDirective` containing only a semantic component ID and a target stage. Server-owned models describe the current revealed-role prefix, compiled visual atoms, and verification receipts. All models must forbid extras, reject non-finite values, remain immutable, and use identifier limits that leave room for deterministic child suffixes.

Second, add `semantic_compiler.py`. The compiler will derive one fixed, centered 3:4 right-triangle construction inside the board's safe area, attach all three squares with vector geometry, place labels in their associated squares, and generate the equation itself. It will compare the requested target stage with the current semantic prefix and produce only missing roles. Each visual atom contains exactly one `put` operation and a deterministic patch ID. A separate verifier function will inspect the produced low-level nodes by semantic role and recalculate the obligations rather than trusting the directive or a model-authored assertion. The compiler returns atoms only after every obligation passes.

Third, add focused tests in `tests/test_live_scene_semantic_contracts.py` and `tests/test_live_scene_semantic_compiler.py`. Tests will cover extra fields, invalid stages and identifiers, no-op and backward transitions, byte-equivalent repeated compilation, absence of clocks/randomness, stable IDs across stages, exact role order, one operation per atom, all board bounds, correct side/square distances, a right-angle dot product, exact compiler-owned LaTeX, conservative label boxes, and output size relative to the raw-coordinate fixture. A fault-injection test will perturb each class of generated geometry and prove the independent verifier fails closed. Another test will interrupt after every atom prefix and show that recompiling toward the same stage emits exactly the missing suffix.

Fourth, integrate the compiler behind a lab-only semantic authoring mode. Add a semantic frame parser and prompt rather than overloading the low-level parser. Update `SceneAuthoringService._stream_attempt()` so one accepted teaching beat can yield several low-level patch events, while the existing patch, revision, wire-size, timeout, repair, and upstream-close limits still apply. Patch budgeting must count compiled visual atoms, not model frames. The initial raw-coordinate mode remains available as a rollback baseline in the lab and stays the production default until the semantic path passes its gate.

Fifth, extend the event metadata and frontend ledger. The server, not the model, attaches the beat ID, atom ID, semantic component ID, role, and verified obligation codes. The browser records an atom only when the existing motion executor includes its node ID in `appliedStepIds`. On interruption, it discards queued atoms, retains the materialized prefix, and sends that semantic prefix with the next request. Replay must reproduce both the low-level scene and semantic atom ledger. No audio field will be fabricated; a future voice adapter can join the visual watermark with actual speech-playback acknowledgements.
...
After the contract and compiler files are added, run:
    ...
    .venv/bin/ruff check backend/murmur/live_scene/semantic_contracts.py backend/murmur/live_scene/semantic_compiler.py tests/test_live_scene_semantic_contracts.py tests/test_live_scene_semantic_compiler.py
...
Runtime acceptance requires zero stale semantic or low-level operations after interruption, exact replay of the committed atom prefix, and a next request whose semantic base contains only acknowledged atoms. Run interruption after every prefix from zero through the final atom, not only at convenient patch boundaries. The final low-level scene and semantic state must be identical regardless of the interruption schedule.
...
The hypothesis is rejected or revised if a semantic beat cannot map cleanly to interruption-safe atoms, verification requires a slow general constraint solver, the component becomes a hard-coded animation with no reusable semantics, or a raw-patch baseline matches its correctness and interruption guarantees with comparable model output and latency. If the vertical slice passes, the next ExecPlan may test three distinct families—analytic plots, graph algorithms, and causal flows—and may define an audiovisual commit watermark with the separate voice pipeline.
