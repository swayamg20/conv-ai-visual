...
The participant token has the minimum grants required to join that room, publish microphone audio and approved data, and subscribe to the agent; it cannot create arbitrary rooms or act as the worker. Signed job metadata carries `user_id`, `session_id`, `agent_id`, `voice_call_id`, runtime/profile version, and trace ID. The worker treats those values as locators, reloads authoritative ownership/configuration, validates data-message schemas/topics, and refuses a mismatch.

The LiveKit worker is a separate entrypoint, `backend/murmur/voice/worker.py`, executed with `uv run python -m murmur.voice.worker dev` locally and `... start` in deployment. It connects outbound to LiveKit Cloud. FastAPI must not own media peers.
...
7. Run the minimal credentialed Cloud slice manually in three terminals.
       ...
       uv run python -m murmur.voice.worker dev
