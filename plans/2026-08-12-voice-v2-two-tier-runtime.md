...
- [ ] 2026-08-19 15:45 IST: Finish the caller-preowned concurrent Coturn evidence drain. Earlier reviews forced one process-level pump claim, a canonical serialized stop receipt, non-copyable proof values, dead-worker reconciliation before hostile Event/clock reads, exact process settlement before parser wipe, graph-opaque cleanup authority, and caller-preowned parser/pump construction. Public secret-bearing `CoturnEvidenceParser.for_probe` has been removed; internal callers now use an identity-bound, serialized, one-shot parser destination and a private no-return initializer. The latest 323-focused/1,274-broad green replacement remains held because exact review reproduced one final P1 family: terminal state becomes observable before the retained process/pump/thread/clock tuple is atomically cleared, so a control between field stores can leave an already-released claim and a permanently wedged partial owner. The next replacement must publish one recoverable terminal snapshot/phase, test every clear boundary, and pass another fresh exact-hash review before commit.
...
- [ ] Compose the reviewed projection, drain, staged app/web/browser process owner, browser observation, and runtime cleanup authorities behind one factory-owned retryable aggregate. Keep the aggregate in a new focused module rather than growing the projection facade or direct-only stack into another monolith.
- [ ] Implement the remaining Checkpoint B slices: owned internal `/29` bridge and digest-pinned non-root Coturn process, generated TLS and bounded trust, secret-safe streaming evidence, relay-only browser/result assertions, exact teardown, then one disposable Linux/amd64 CI proof before removing the relay refusal. No host-network or private-aiortc fallback is permitted implicitly.
