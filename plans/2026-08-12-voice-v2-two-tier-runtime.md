...
Milestone 1B dependency and contract work has begun but has not crossed a media gate. Commit `d11de75` pins Pipecat 1.7, its direct provider integrations, SmallWebRTC, and the exact browser SDK pair, while `de068ff` defines the common claims/assignment/lifecycle and guarded qualification boundaries. The existing legacy `/offer` code remains explicitly outside the challenger. The open work is the authenticated signaling application, owned pipeline, browser adapter, deterministic fake media, explicit Coturn configuration, direct and forced relay browser paths, authoritative cleanup, and its complete cost record.
