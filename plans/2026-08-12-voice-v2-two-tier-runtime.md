...
The relay work is split at an honesty boundary. Checkpoint A owns only the pre-import contract: exact network enum, digest/platform/URL, a secret-free candidate configuration, private generated config and CA validation, claim-bounded REST credentials, guarded app projection, direct compatibility, and categorical refusal of every `relay-tls` runner invocation before side effects. The checked-in candidate uses `user-quota=2` and `total-quota=2` because the one-call, RTCP-mux harness expects one allocation for each endpoint, potentially under one REST username; it also uses `no-tcp-relay` so peer-side relay stays UDP while client-to-TURN remains TLS/TCP. Those directives and the pinned 4.17.2 configuration remain runtime-unvalidated. Do not add lifetime, bandwidth, or other flags from memory; verify their exact pinned semantics first in Checkpoint B.
