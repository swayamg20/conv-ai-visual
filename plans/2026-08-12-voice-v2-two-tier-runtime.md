...
- [ ] 2026-08-16 01:02 IST: Implement Checkpoint B in reviewed slices. First add the Docker-free E2E-only browser policy hook and preserve direct-mode constructor behavior exactly. In parallel, validate an owned Docker bridge candidate with loopback-only published TLS/UDP ports, no host networking, no `allow-loopback-peers`, no private aiortc patch, and no relay claim until a disposable Linux/amd64 CI run proves the routing, pinned Coturn grammar, allocation/traffic, and teardown contracts.
