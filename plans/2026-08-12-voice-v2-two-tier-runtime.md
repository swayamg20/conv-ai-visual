...
The pinned Coturn image's Docker tag points to revision `34230574e952f51858f2f2166dab162d448e7809`, whose official build resolves Coturn release `4.17.2` to binary-source commit `de0c9b28f22281a0251d7e98aa8a097895a5b185`. That source showed that the allocation, usage, refresh, close, and delete records needed for evidence are gated by lowercase `verbose`; `simple-log` does not enable them. It also showed that `no-dtls` is deprecated and behaviorally redundant because DTLS is opt-in in this release. The executable candidate must therefore add `verbose` plus `log-min-level=info`, remove `no-dtls`, keep attached output memory-only and bounded to the source's 1024-byte record maximum, and validate the real runtime output before any grammar is accepted.
