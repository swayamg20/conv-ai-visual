...
- Decision: Retarget parameters in place at a settled frontier and preserve physical node IDs, component ID, frontier, and applicable clarification flags. Rationale: this directly proves a living model rather than a template restart. A same-spec target declines as `no_forward_progress`; an unannounced problem change on advance or clarify declines as `problem_conflict`. Date/author: 2026-09-11, Codex.
...
Third, implement the deterministic compiler in `projectile_motion_compiler.py`. It will author the six-checkpoint board with value-neutral stable IDs for axes, ground, launch vector, component vectors, acceleration, ascent path, descent path, projectile, apex, range, and formula anchors. Every supported trajectory uses the same sample count and path topology. The default cinematic board uses one fixed physical plot spanning all nine problems, with a separately qualified compact viewport. Dynamic numerical labels use parameter-specific IDs and enter/exit cross-fades during retarget. The compiler targets a 35-to-45-second normal lesson and keeps any single unexplained hold below 1.2 seconds.
