...
No runtime code has shipped under this plan yet. The planning checkpoint establishes a reviewed implementation boundary: capability and provenance first, process ownership second, executor integration third, and real disposable-Linux qualification only after explicit authorization. It rejects the tempting shortcuts of flag flipping, `_ManagedProcess` reuse, fixed-port readiness, and leader-only exit checks.
