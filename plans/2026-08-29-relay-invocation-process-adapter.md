...
- [x] 2026-08-29 18:05 IST: Shipped `96c14de`, the effect-free concrete-pair transaction and its private executor binding. The exact consumed build now mints one cap-one grant/driver/tools pair, binds the complete owner and app/web/browser authority graph, revokes forward callbacks when cleanup latches, retires in `live -> cleanup -> retiring -> cleared` order, seals and scrubs every destination/token/callback, and reconciles pair construction and retirement return loss without refreshing runtime or cleanup deadlines. Before pair construction, the executor now publishes a replay-safe `preparing` authority atomically with its result destination: live caller objects are weakly identity-bound, the bounded static secret is retained only as a domain-separated keyed digest, and malformed or expired replay state fails closed. Cleanup authority resolution now recovers a missing primary entry only from its exact weak owner proof, requires primary/weak identity agreement, and distinguishes terminal idempotence with a weak tombstone. Two independent final adversarial reviews report zero P0/P1 findings. The 423-test invocation/executor/public-refusal gate passes with warnings as errors; Ruff, formatting, byte-compilation, diff, and both scoped module-cap gates pass. This checkpoint performs no new subprocess, filesystem, HTTP, browser, Docker, network, or media effect.
- [x] Checkpoint 1: add failing boundary tests, consumed-build invocation provenance, and sealed matched concrete driver/tools construction with no OS or HTTP effects.
...
Pair construction exposed a second durable-input boundary before live inner evidence exists. Publishing a bare result destination and then attaching replay identity is insufficient: return loss at that store lets a changed caller adopt the already-minted pair. The result destination must therefore contain the complete replay-safe tuple before its first registry publication, and that `preparing` tuple must be the exact tuple later promoted into live and terminal authority state.

Python object addresses are not durable identity. A keyed hash of `id(type(value))` and `id(value)` admits ABA after the original objects die and CPython reuses their addresses. Replay now holds weak references and requires `reference() is candidate` for runner, bridge probe, trusted tools, synthetic selection, and clocks; the immortal concrete selector is checked against its singleton. The static authentication secret is value-semantic, bounded UTF-8 and represented only by a domain-separated keyed digest. Dead references and every malformed exact-value shape reject without leaking a raw attribute error.

A missing cleanup registry entry is not proof of terminal cleanup, and a primary registry entry alone is not enough to choose an owner. The cleanup authority now has a second weak exact-owner record plus a weak terminal tombstone. Missing-primary recovery uses only the exact weak owner; a primary/weak crosswire fails closed; terminal no-op succeeds only after the tombstone is committed and the primary entry is absent.
...
2026-08-29, Codex: Treat the concrete pair and its complete invocation-owner graph as one private capability transaction. The public descriptive `concrete_adapter` property is true only while the exact canonical pair is live; cleanup latching revokes cached forward callbacks; retirement unpublishes before graph sealing, uses the global `construction_lock -> operation_lock -> pair effect lock` order, and scrubs driver/tools only after the exact graph reaches terminal state. Every commit boundary is reconciled by rereading the same identity-bound state.

2026-08-29, Codex: Bind replay-safe inner call authority before any concrete pair state. Publish the result destination only after its 14-slot tuple is attached, retain live object identity through weak references rather than addresses, represent the static secret with a bounded keyed digest, and promote that same tuple from `preparing` to live to terminal. Cleanup authority resolution likewise requires agreement between the primary entry and a weak exact-owner proof; only a separately committed terminal tombstone authorizes an absent-owner no-op.

## Outcomes & Retrospective

The first effect-free runtime substrate shipped as `e324eb6`, the consumed-build execution proof shipped as `8e69821`, and the completed concrete capability transaction shipped as `96c14de`. Checkpoint 1 now supplies the exact canonical grant/driver/tools pair, complete owner-graph binding, replay-safe preauthority, and reverse retirement/scrubbing transaction while keeping every new callback inert. It still creates no child, readiness request, browser, Docker, network, or media observation. The implementation boundary remains capability and provenance first, process ownership second, fake-local executor integration third, and real disposable-Linux qualification only after explicit authorization. It rejects flag flipping, `_ManagedProcess` reuse, fixed-port readiness, address-hash identity, bare registry absence as terminal proof, digest-only live authority, and leader-only exit checks.
