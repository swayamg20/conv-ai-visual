...
- [ ] Remove LiveKit production dependencies, infrastructure, secrets, worker sidecar, and browser imports after the replacement path passes equivalent acceptance.
- [ ] Deploy the exact pushed replacement commit and run one explicitly budgeted authenticated live call.

## Surprises & Discoveries

- The current merged `main` contains a complete but dormant LiveKit Cloud deployment path. It is not active in Azure, so this migration can proceed as a follow-up without disrupting the currently deployed legacy runtime.
...
`infra/azure/apps.bicep`, `scripts/deploy_azure.py`, `deploy/backend.Dockerfile`, `requirements.txt`, `pyproject.toml`, and `web/Dockerfile` currently contain production LiveKit selections added by the dormant deployment PR. They remain untouched until the first-party path passes local and Azure transport acceptance; then they will be simplified in one reviewable migration rather than prematurely restoring the broken legacy deployment.
